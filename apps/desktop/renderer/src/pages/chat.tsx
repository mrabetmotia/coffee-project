...
function useChatConversation(conversationId: string | undefined) {
  ...
  useEffect(() => {
    ...
    const onConnect = () => {
      ...
      setConnected(true);
      if (authenticatedRef.current) socket.emit('chat:conversation:join', { conversationId });
    };
  ...
  return { messages, connected, typing, presence, content, sending, sendError, bottomRef, emitTyping, send };
}
...
export function ClientChatPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Messages" subtitle="Votre équipe CaféStock est là pour vous aider." /><ChatWindow title="Support CaféStock" subtitle="Nous répondons généralement rapidement" online={chat.presence || chat.connected} messages={chat.messages} currentUserId={getCurrentUserId()} typing={chat.typing} connected={chat.connected} content={chat.content} sending={chat.sending} bottomRef={chat.bottomRef} onChange={chat.emitTyping} onSubmit={chat.send} /></div>;
}
